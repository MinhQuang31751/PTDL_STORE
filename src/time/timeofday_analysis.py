...
def create_monthly_revenue_chart(df: pd.DataFrame):
    ...
    fig = px.line(monthly_sales, x='YearMonth', y='TotalPrice', title='Xu hướng Doanh thu Hàng tháng', markers=True)
...
def display_revenue_stats(df: pd.DataFrame):
    ...
    total_revenue = monthly_sales['TotalPrice'].sum()
    avg_revenue = monthly_sales['TotalPrice'].mean()
