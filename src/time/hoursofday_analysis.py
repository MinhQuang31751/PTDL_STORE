...
def create_hourly_revenue_chart(df: pd.DataFrame):
    ...
    # Sử dụng cột InvoiceHour và TotalPrice từ dataset
    hourly_sales = df[df['InvoiceHour'].between(6, 20)].groupby('InvoiceHour')['TotalPrice'].sum().reset_index()
...
def display_hourly_stats(df: pd.DataFrame):
    ...
    total_revenue = hourly_sales['TotalPrice'].sum()
    avg_revenue = hourly_sales['TotalPrice'].mean()
    max_revenue = hourly_sales['TotalPrice'].max()
    min_revenue = hourly_sales['TotalPrice'].min()
