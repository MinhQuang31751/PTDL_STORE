...
def create_weekday_revenue_bar_chart(df: pd.DataFrame):
    ...
    weekday_sales = df.groupby('InvoiceDayName')['TotalPrice'].sum().reset_index()
    ...
    fig = px.bar(weekday_sales, x='InvoiceDayName', y='TotalPrice', 
...
def create_weekday_revenue_pie_chart(df: pd.DataFrame):
    ...
    fig = px.pie(weekday_sales, values='TotalPrice', names='InvoiceDayName', 
...
def display_weekday_stats(df: pd.DataFrame):
    ...
    total = weekday_sales['TotalPrice'].sum()
    avg = weekday_sales['TotalPrice'].mean()
    best_day = weekday_sales.loc[weekday_sales['TotalPrice'].idxmax(), 'InvoiceDayName']
    worst_day = weekday_sales.loc[weekday_sales['TotalPrice'].idxmin(), 'InvoiceDayName']
